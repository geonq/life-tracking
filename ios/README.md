...
This is the product track for the iPhone and Mac Life OS. It includes a dark-mode-first adaptive branded interface, a writable calendar/task system with emoji or bounded local PNG/JPEG icons, and source-backed, provenance-first dashboards. The React dashboard remains a visual/reference prototype only.
...
- HealthKit reads and the narrowly typed, authorization-gated write path are iPhone main-app only. Life OS requests read access only through the explicit Settings prompt for selected source-backed fitness, sleep, activity, hydration, and caffeine categories, and separately requests write authorization only for supported user-authored types (water, caffeine, body mass, body-fat percentage, and lean body mass). A successful request is not treated as proof that a sample was saved; every write is rechecked against the current per-type sharing authorization and the HealthKit save result. Per-type read authorization remains unknowable, so Life OS makes no per-type read-permission claim and does not treat an empty read as proof of denial or no data. Widgets, widget snapshot tests, macOS, and the Mac widget exclude HealthKit sources. The supported provenance path is Helio → Zepp → Apple Health → HealthKit, and physical-device proof is required before claiming Helio provenance or permission behavior.
- Calendar remote writes are conditional and idempotent at the gateway. The authority envelope is the single commit point; compatibility projections and WebSocket fan-out are repairable after a commit. The idempotency journal is a bounded rolling replay window, so recent retries remain replays across restart while expired keys are subject to the current ETag check rather than being retained forever.
...
- App Group entitlements are source-configured for app/widget targets using `$(APP_GROUP_IDENTIFIER)` and the stable `group.com.hermes.lifeos` identifier. The identifier is not a secret, but the signed app/widget profiles must still provision that exact group. SideStore/AltStore may rewrite entitlements during free sideloading; inspect the installed result rather than assuming the source identifier survives unchanged.
...
- Visual fixture hosts pass `usesVisualFixtures` explicitly. Fixture coordinators do not open live Calendar WebSockets or publish widget snapshots, keeping macOS screenshots deterministic and offline; this does not substitute for signed-device/background evidence.
