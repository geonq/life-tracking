...
final class PlanningInteractionTests: XCTestCase {
    ...
    func testIOSCancellationInvalidatesTokenAndFreshTouchCanStart() {
        var bridge = PlanningGestureBridge(platform: .iOS)
        XCTAssertEqual(
            bridge.beginPointer(at: .zero, nodeID: "node", timestamp: 10),
            .pendingNodeLongPress
        ...
        XCTAssertTrue(
            bridge.cancel(sequenceID: cancelledSequence, owner: .pendingNodeLongPress)
        )
        XCTAssertEqual(bridge.owner, .cancelled)
        XCTAssertFalse(
            bridge.finish(sequenceID: cancelledSequence, owner: .pendingNodeLongPress)
        )

        XCTAssertEqual(
            bridge.beginPointer(at: CGPoint(x: 20, y: 20), nodeID: "next", timestamp: 20),
            .pendingNodeLongPress
        )
        XCTAssertGreaterThan(bridge.sequenceID, cancelledSequence)
        let freshSequence = bridge.sequenceID
        XCTAssertTrue(bridge.finish(sequenceID: freshSequence, owner: .pendingNodeLongPress))
        XCTAssertEqual(
            bridge.beginPointer(at: .zero, nodeID: nil, touchCount: 2, timestamp: 20),
            .zoom
        )
    }
