...
public struct CalendarWidgetView: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            else { ForEach(upcoming) { item in HStack(spacing: 7) { CalendarIconView(item: item); VStack(alignment: .leading) { Text(item.title).font(.caption.bold()).lineLimit(1); Text("\(item.start, format: .dateTime.hour().minute())–\(item.end, format: .dateTime.hour().minute())").font(.caption2).foregroundStyle(.secondary) }; Spacer(); LifeOSIcon(item.status == .done ? .done : item.status == .aborted ? .aborted : item.status == .inProgress ? .inProgress : .planned).frame(width: 14, height: 14).foregroundStyle(item.status.color).accessibilityLabel(item.status.label) } } }
        }.containerBackground(for: .widget) { LifeOSTokens.surface }.accessibilityElement(children: .combine).accessibilityLabel("Calendar for \(entry.date, format: .dateTime.month().day()). \(upcoming.count) upcoming items.")
    }
}
