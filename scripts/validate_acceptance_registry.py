...
def _producing_sha_default(metadata: Mapping[str, Any]) -> str | None:
    ...
    The implementation baseline and the commit that produces this registry
    are intentionally separate. ``registry_producing_commit`` is the audited
    metadata fallback for focused callers; CI can supply an explicit expected
    commit instead. Before that commit exists the registry remains UNFROZEN and
    rows can only be missing/foundation/blocked.
    """
...
def _canonical_hash_payload(data: Mapping[str, Any]) -> bytes:
    ...
    payload.pop("frozen_registry_sha256", None)
...
def _git_repository_available(repo_root: Path | None) -> bool:
    return repo_root is not None and (repo_root / ".git").exists()
...
def _path_exists(path_value: Any, repo_root: Path | None) -> bool:
    ...
    if stable_path.startswith("repo://"):
        stable_path = stable_path.removeprefix("repo://")
    ...
    *,
    ...
    for row in rows:
        ...
        if status == "accepted":
            ...
            elif check_evidence_paths and not _path_exists(evidence_path, repo_root):
                errors.append(f"{row_id}: accepted evidence path does not exist: {evidence_path}")
            current_commit = resolved_expected_commit or materialised.get("registry_producing_commit")
            producing_identity = _resolve_commit_reference(producing_sha, repo_root) or str(producing_sha).lower()
            expected_identity = _resolve_commit_reference(current_commit, repo_root) if current_commit is not None else None
            if not _nonempty(current_commit) or expected_identity is None or producing_identity != expected_identity:
                commit_label = "expected commit" if resolved_expected_commit else "current producing commit"
                errors.append(f"{row_id}: accepted row does not have evidence at {commit_label}")
    ...
    if producing_commit is not None and not SHA_RE.fullmatch(str(producing_commit)):
        errors.append(f"invalid registry-producing commit {producing_commit!r}")
    if state == "UNFROZEN" and any(row.get("status") == "accepted" for row in rows):
        errors.append("accepted rows require a registry_producing_commit and a FROZEN registry")
    if state == "FROZEN":
        if not _nonempty(producing_commit):
            errors.append("FROZEN registry requires registry_producing_commit")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="also require accepted evidence paths to exist on disk",
    )
    parser.add_argument(
        "--expected-commit",
        default=None,
